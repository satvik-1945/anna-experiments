...
- [x] Use a real or realistic profile (name, role, short `.tex` sample)
- [x] Pre-fetch once before recording so the demo path is fast; re-fetch live only if time allows
- [x] Hide Boost scrape section unless you want to mention "coming later"
- [x] Window size ~900×950 so the sidebar and table look clean
- [x] No terminal visible — Anna UI only
